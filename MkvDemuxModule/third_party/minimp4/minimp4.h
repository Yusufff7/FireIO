...
// PATCH (StremioVega/MkvDemuxModule): AC-3 / Enhanced AC-3. These are NOT
// real ISO/IEC systems object-type values — AC-3 and E-AC-3 don't use the
// MPEG-4 ES-descriptor scheme (esds) at all, so there is no registry value
// to reuse. They exist purely as internal sentinels the audio sample-entry
// writer below checks for, to pick the `ac-3`/`ec-3` box + `dac3`/`dec3`
// box path instead of `mp4a`+`esds`. Values chosen simply to avoid
// colliding with any MP4_OBJECT_TYPE_* above.
#define MP4_OBJECT_TYPE_AC3                                    0xA5
#define MP4_OBJECT_TYPE_EAC3                                   0xA6
// Same story as AC-3/E-AC-3 above, not a real registry value: FLAC gets its
// own `fLaC` sample entry + `dfLa` config box (ISO/IEC 14496-12 Amendment
// 2's FLACSpecificBox), not mp4a+esds.
#define MP4_OBJECT_TYPE_FLAC                                   0xA7
...
/**
*   PATCH (StremioVega/MkvDemuxModule): supplies the real profile/tier/level
*   fields for a HEVC track's hvcC box. Without this call the writer falls
*   back to hardcoded placeholder values. See the `hevc_ptl_set` field on
*   track_t for why this exists.
*
*   return error code MP4E_STATUS_*
*/
int MP4E_set_hevc_ptl(MP4E_mux_t *mux, int track_id, unsigned profile_space, unsigned tier_flag,
    unsigned profile_idc, unsigned profile_compat_flags, uint64_t constraint_flags, unsigned level_idc);
...
enum
{
    ...
    // PATCH (StremioVega/MkvDemuxModule): AC-3 / E-AC-3 sample entry + config
    // boxes (ETSI TS 102 366 Annex F). Not part of stock minimp4.
    BOX_ac3     = FOUR_CHAR_INT( 'a', 'c', '-', '3' ),
    BOX_dac3    = FOUR_CHAR_INT( 'd', 'a', 'c', '3' ),
    BOX_ec3     = FOUR_CHAR_INT( 'e', 'c', '-', '3' ),
    BOX_dec3    = FOUR_CHAR_INT( 'd', 'e', 'c', '3' ),
    BOX_flaC    = FOUR_CHAR_INT( 'f', 'L', 'a', 'C' ),
...
typedef struct
{
    ...
    // PATCH (StremioVega/MkvDemuxModule): the stock hvcC writer below hardcodes
    // placeholder profile/tier/level bytes ("TODO: read actual params from
    // stream") instead of the real values from the encode. MP4E_set_hevc_ptl()
    // lets a caller who already has these — e.g. parsed from the Matroska
    // CodecPrivate's HEVCDecoderConfigurationRecord, which carries them
    // verbatim — provide the real ones instead.
    unsigned char hevc_ptl_set;
    unsigned char hevc_general_profile_space;
    unsigned char hevc_general_tier_flag;
    unsigned char hevc_general_profile_idc;
    ...
    unsigned profile_idc, unsigned profile_compat_flags, uint64_t constraint_flags, unsigned level_idc)
{
    ...
    assert(tr->info.track_media_kind == e_video);
    tr->hevc_ptl_set = 1;
    tr->hevc_general_profile_space = (unsigned char)profile_space;
    tr->hevc_general_tier_flag = (unsigned char)tier_flag;
    tr->hevc_general_profile_idc = (unsigned char)profile_idc;
    tr->hevc_general_profile_compat_flags = profile_compat_flags;
    tr->hevc_general_constraint_flags = constraint_flags;
    tr->hevc_general_level_idc = (unsigned char)level_idc;
            ...
            ATOM(BOX_mdia);
                ...
                ATOM(BOX_minf);
                    ...
                    ATOM(BOX_stbl);
                        ...
                        if (tr->info.track_media_kind == e_audio || tr->info.track_media_kind == e_private)
                        {
                            // PATCH (StremioVega/MkvDemuxModule): AC-3/E-AC-3/
                            // FLAC don't use the MPEG-4 ES-descriptor scheme
                            // (mp4a+esds) at all — each gets its own sample
                            // entry box with a flat, format-specific config
                            // box instead (dac3/dec3/dfLa).
                            int is_raw_config_box = tr->info.track_media_kind == e_audio &&
                            ...
                            if (tr->info.track_media_kind == e_audio)
                            {
                                if (tr->info.object_type_indication == MP4_OBJECT_TYPE_EAC3)
                                {
                                    ATOM(BOX_ec3);
                                } else if (tr->info.object_type_indication == MP4_OBJECT_TYPE_AC3)
                                {
                                    ATOM(BOX_ac3);
                                } else if (tr->info.object_type_indication == MP4_OBJECT_TYPE_FLAC)
                                {
                                    ATOM(BOX_flaC);
                            ...
                            {
                                // PATCH (StremioVega/MkvDemuxModule): the
                                // pre-parsed/pre-extracted config bytes,
                                // handed in via MP4E_set_dsi the same way
                                // AAC's AudioSpecificConfig is — reusing
                                // that storage (and its leading 2-byte size
                                // prefix) rather than adding a parallel
                                // field just for this. dac3/dec3 are a
                                // packed bitstream-info struct the caller
                                // built; dfLa is the FLAC STREAMINFO
                                // metadata block copied straight through.
                                //
                                // dfLa is a FullBox (ISO/IEC 14496-12
                                // Amendment 2's FLACSpecificBox extends
                                // FullBox('dfLa', 0, 0)) — 4 extra
                                // version+flags bytes before its payload
                                // that dac3/dec3 (plain Box, not FullBox)
                                // don't have. Missing this produced a
                                // dfLa box ffmpeg's own demuxer rejected
                                // outright ("error reading header") because
                                // the metadata-block bytes ended up
                                // misaligned by 4 — confirmed by comparing
                                // the failing box's raw bytes against a real
                                // FLAC STREAMINFO layout.
                                // dfLa is the only FullBox of the four. dac3
                                // and dec3 are plain Boxes, and so is dOps —
                                // dOps looks like a FullBox because its first
                                // payload byte is a Version, but that byte
                                // belongs to OpusSpecificBox itself (the
                                // caller writes it), NOT to a FullBox header.
                                // Emitting it with ATOM_FULL would insert 4
                                // spurious bytes and shift every field after
                                // it — a decoder would read a bogus channel
                                // count and pre-skip.
                                int is_flac = tr->info.object_type_indication == MP4_OBJECT_TYPE_FLAC;
...
#define WRITE_OD_LEN(size) if (size > 0x7F) do { size -= 0x7F; WRITE_1(0x00ff); } while (size > 0x7F); WRITE_1(size)
                        ...
                        if (tr->info.track_media_kind == e_video && (MP4_OBJECT_TYPE_AVC == tr->info.object_type_indication || MP4_OBJECT_TYPE_HEVC == tr->info.object_type_indication))
                        {
                            ...
                            {
                                // PATCH (StremioVega/MkvDemuxModule): was `items_count(&tr->vpps)`
                                // — the wrong vector (PPS count, not VPS count) for this field.
                                int numOfVPS  = items_count(&tr->vvps);
                                ATOM(BOX_hvcC);
                                if (tr->hevc_ptl_set)
                                {
                                    // PATCH (StremioVega/MkvDemuxModule): real values from the
                                    // source encode (see MP4E_set_hevc_ptl) instead of the
                                    // hardcoded placeholders below.
                                    WRITE_1(1);    // configurationVersion
                                    WRITE_1(((tr->hevc_general_profile_space & 0x3) << 6) |
                                            ((tr->hevc_general_tier_flag & 0x1) << 5) |
                                            (tr->hevc_general_profile_idc & 0x1f));
                                    WRITE_4(tr->hevc_general_profile_compat_flags);
                                    WRITE_2((unsigned)((tr->hevc_general_constraint_flags >> 32) & 0xffff));
                                    WRITE_4((unsigned)(tr->hevc_general_constraint_flags & 0xffffffffu));
                                    WRITE_1(tr->hevc_general_level_idc);
                                } else
                                {
